...
def server_edit(request, asset):
    ...
    if device_form.is_valid() and server_form.is_valid() and maninfo_form.is_valid():
        ...
        modinfo(type="服务器", asset= asset, field_list=device_change_fields, \
                old_obj=old_device_instance, new_obj=Devices.objects.get(asset=asset), \
                username=request.user)
        modinfo(type="服务器", asset= asset, field_list=server_change_fields, \
                old_obj=old_server_instance, new_obj=Server.objects.get(asset=asset), \
                username=request.user)
        modinfo(type="服务器", asset= asset, field_list=maninfo_change_fields, \
                old_obj=old_maninfo_instance, new_obj=ManInfo.objects.get(asset=asset), \
                username=request.user)
